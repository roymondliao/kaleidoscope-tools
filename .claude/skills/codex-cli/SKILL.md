...
1. Ask the user (via `AskUserQuestion`) which model to run and which reasoning effort to use (`xhigh`, `high`, `medium`, or `low`) in a **single prompt with two questions**. Models list:
   - `gpt-5.3-codex`
   - `gpt-5.2-codex`
   - `gpt-5.2`
   - `gpt-5.1-codex-max`
