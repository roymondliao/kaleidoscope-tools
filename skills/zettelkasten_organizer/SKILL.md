...
description: >
  ...
  normalize filenames (YYYYMMDDHHmm-Slug), merge frontmatter (supports both YAML and blockquote Metadata),
...
This skill follows the conventions defined in `zettelkasten_guidelines.md`:
- Note types (MOC / Literature / Permanent)
- Naming rules (`YYYYMMDDHHmm-{Slug}.md`, `YYYYMMDD-{ShortTitle}.md`, `MOC-{Topic}.md`)
- Metadata format (blockquote Metadata)
- Folder structure
...
- Example: `202601161400-CoT降低映射複雜度.md`

Special patterns by note type:
- MOC: `MOC-{Topic}.md` (no date prefix)
- Literature Note: `YYYYMMDD-{ShortTitle}.md` (8-digit date, covers papers/blogs/videos)
- Permanent Note: `YYYYMMDDHHmm-{Slug}.md` (12-digit timestamp for finer granularity)
...
- keep Chinese characters as-is (no transliteration)
...
The system supports TWO metadata formats:

## Format A: YAML Frontmatter (machine-friendly)
...
tags: []
aliases: []
previous: "[[wikilink]]"    # thinking chain source
source: "[[wikilink]]"      # evidence source (literature note)
related: []                 # related wikilinks
```

## Format B: Blockquote Metadata (research-friendly)

```markdown
> **Metadata**
> - **Type**: #Permanent | #Literature
> - **Previous (思考脈絡)**: [[202512011000-Prompt工程基礎]]
> - **Source (證據來源)**: [[20260116-Paper-Wei_et_al_CoT]]
> - **Related (相關連結)**:
>   - [[202601150900-InContextLearning機制]]
>   - [[202601101100-人類推理心理學]]
> - **Tags**: #LLM #CoT #Reasoning
```

## Parsing Rules

- If note has YAML frontmatter → use Format A
- If note has `> **Metadata**` blockquote → parse as Format B
- If both exist → YAML frontmatter takes precedence for `type`/`status`; merge other fields
...
- `source`: required for literature notes; if missing, report and plan patch (MED).
- `previous`: recommended for permanent notes (thinking chain).
- `related`: optional, helps build knowledge graph.
...
# Research Note Types (from Research Zettelkasten)

The system recognizes four core research note types:

| Note Type | Definition | Folder | Filename Pattern |
|-----------|------------|--------|------------------|
| **MOC (Index)** | Research question hub, links related notes | `30_Maps/` | `MOC-{Topic}.md` |
| **Literature Matrix** | Comparison table for scanning sources | `40_Projects/` | `Matrix-{Topic}.md` |
| **Literature Note** | Single source record (paper/blog/video) | `10_Literature/` | `YYYYMMDD-{ShortTitle}.md` |
| **Permanent Note** | Your own insights and claims | `20_Permanent/` | `YYYYMMDDHHmm-{Slug}.md` |

# Type → Folder Mapping

Frontmatter `type` values map to folders:

- inbox → `00_Inbox/`
- daily → `01_Daily/`
- literature → `10_Literature/` (includes Literature Notes)
- permanent → `20_Permanent/` (includes Permanent Notes)
- map → `30_Maps/` (includes MOCs)
- project → `40_Projects/` (includes Literature Matrix)
- archive → `90_Archive/`

Type inference rules:
- Has `> **Metadata**` blockquote with `Type: #Permanent` → permanent
- Has `> **Metadata**` blockquote with `Type: #Literature` → literature
- Filename starts with `MOC-` → map
- Filename starts with `Matrix-` → project
- Has `source`/`url`/`doi`/`isbn` field → literature
...
- If classification is reliable:
  - generate `frontmatter` + `move` (cross-top => HIGH)

## Reliable classification signals

| Signal | Inferred Type | Confidence |
|--------|---------------|------------|
| Blockquote `Type: #Permanent` | permanent | HIGH |
| Blockquote `Type: #Literature` | literature | HIGH |
| YAML `type: permanent` | permanent | HIGH |
| YAML `type: literature` | literature | HIGH |
| Has `source`/`url`/`doi`/`isbn` field | literature | HIGH |
| Filename `YYYYMMDD-*.md` (8-digit date) in `10_Literature/` | literature | MED |
| Has `## Core Claim` or `## 核心主張` section | permanent | MED |
| Has `Previous` or `Source` wikilinks | permanent | MED |
| Filename starts with `MOC-` | map | HIGH |
| Filename starts with `Matrix-` | project | MED |

Only HIGH confidence signals trigger automatic move ops.
...
- STATUS-MISSING
- STATUS-INVALID
- SOURCE-MISSING (required for literature)
- PREVIOUS-MISSING (recommended for permanent, severity: LOW)
- RELATED-EMPTY (info only, no action needed)
...
- "幫我整理一下 Obsidian vault / 筆記庫"
- "掃描 Obsidian，輸出 Report.md + 可執行 Plan.md（先別改）"
- "把檔名統一成 YYYYMMDDHHmm-Slug 格式"
- "批量補全/規範 frontmatter（type/status/previous/source/related）"
- "找孤兒筆記（0入鏈且不在任何 MOC）、斷鏈、重複標題、未引用附件"
- "重命名/移動後修復 wikilinks / markdown links"
- "按研究型 Zettelkasten（Literature/Permanent/MOC）整理"
- "附件放到每個頂層資料夾的 \_assets 並修復引用"
- "生成整理計畫並可回滾（90_Archive/\_organizer_backups）"
- "檢查永久筆記是否缺少 Previous/Source 連結"

Keywords that strongly indicate this skill:

- Zettelkasten, Research Zettelkasten, MOC, Literature Note, Permanent Note
- 00_Inbox/10_Literature/20_Permanent/30_Maps
- \_assets, dry-run, Plan.md executable, backup rollback
- Previous, Source, Related, blockquote Metadata
